'''
NB:
This code can get the computational time for proposed approach under different problem sizes, just simply replace the transmission line parameter values
...
for kk in range(ITERATION):
    ...
    for ll in range(ITERATION):
        ...
        # replace the optimal pi from the approach
        for r in range(number_R):
            for r_prime in range(number_R):
                for k in range(K):
                    if r==1 and r_prime==3 and k==3:
                        prob8 += var_pi[r][r_prime][k] == 1
                    elif r==0 and r_prime==1 and k==4:
                        prob8 += var_pi[r][r_prime][k] == 1
